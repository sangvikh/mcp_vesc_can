...
int vesc_can_read() {
  ...
  if (!CAN.readMsgBuf(&inMsg.id, &inMsg.ext, &inMsg.len, inMsg.buf)) {
...
bool sendPacket(uint8_t id, uint8_t packet[], int32_t len) {
  CAN_message_t  msg;
  msg.id = id;
  msg.ext = 1;    //The VESC needs an extended ID
  msg.len = len;
  memcpy(msg.buf, packet, len * sizeof(uint8_t));
  msg.timeout = 0;
  return (bool) CAN.sendMsgBuf(msg.id, msg.ext, msg.len, msg.buf);
}
